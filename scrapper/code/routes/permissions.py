...
def list_permissions(db: Session = Depends(get_db), current_user: models.User = Depends(admin_required)):
    perms = db.query(models.Permission).all()
    return perms


@router.post("", response_model=schemas.PermissionOut, status_code=status.HTTP_201_CREATED)
def create_permission(payload: schemas.PermissionCreate, db: Session = Depends(get_db), current_user: models.User = Depends(admin_required)):
...
def delete_permission(permission_id: str, db: Session = Depends(get_db), current_user: models.User = Depends(admin_required)):
